...
import {
  ...
  GET_REPOSTS,
} from "../graphql/queries/postsQueries";
import RepostList from "../components/Repost/RepostList";
...
const Profile = () => {
  ...
  const { data: getReposts } = useQuery(GET_REPOSTS(id));
  ...
  useEffect(() => {
    if (getReposts) {
      setReposts(
        getReposts.usersPermissionsUser?.data?.attributes?.reposts?.data[0]
          ?.attributes?.post?.data
      );
    }
  });
  ...
  useEffect(() => {
    if (getFollow.data) {
      const newFollows = [];
      getFollow.data.usersPermissionsUser.data.attributes.follows.data.forEach(
        (element) => {
  ...
  if (data) {
    return (
      <>
        <Buttons variant="text" onClick={createNewChat}>
          + Conversation
        </Buttons>
        <div className="profilPageContent">
          <div className="profil">
            ...
            <div className="userInfos">
              ...
              {user.id !== id && (
                <Button
                  value={follows.includes(id) ? "Ne plus suivre" : "Suivre"}
                  className="bold"
